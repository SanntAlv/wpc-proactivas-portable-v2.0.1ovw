...
#>
    ...
    Begin {
        [PSCustomObject[]] $hcl = (Get-Content -Path (".\automatizaciones\lib\data\vmware-hcl.json") | ConvertFrom-Json)
        [PSCustomObject[]] $esxiReleases = (Get-Content -Path (".\automatizaciones\lib\data\esxiReleases.json") | ConvertFrom-Json)
    ...
    Process {
        $AllInfo = @()
            ...
            $Info = "" | Select-Object VMHost, Build, ReleaseLevel, Manufacturer, Model, Cpu, Supported, SupportedReleases, Reference, Note
            ...
            $release = $esxiReleases.data.esxiReleases | Where-Object build -eq $vmHost.Build
            if ($release) {
