...
$devices = Get-VMHost | Get-VMHostPciDevice | Where-Object { $_.DeviceClass -eq "MassStorageController" -or $_.DeviceClass -eq "NetworkController" -or $_.DeviceClass -eq "SerialBusController"} 
...
Foreach ($device in $devices) {
  ...
  $DeviceFound = $false
  $Info = "" | Select-Object VMHost, Device, DeviceName, VendorName, DeviceClass, vid, did, svid, ssid, Driver, DriverVersion, FirmwareVersion, VibVersion, Supported, Reference
  ...
  if($DeviceFound){
    if ($device.DeviceClass -eq "NetworkController"){
      ...
      $vmnicId = $niclist | Where-Object { $_.PCIDevice -like '*'+$device.Id}
      ...
      $vmhbaId = $device.VMHost |Get-VMHostHba -ErrorAction SilentlyContinue | Where-Object { $_.PCI -like '*'+$device.Id} 
